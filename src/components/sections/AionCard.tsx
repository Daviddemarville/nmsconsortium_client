...
type Props = {
  dataUrl?: string; // "/data/aion_meta.json"
  membersUrl?: string; // "/data/members.json"
  rolesUrl?: string; // "/data/roles.json"
  roleIds?: number[];
  roleIncludes?: string[]; // ex: ["médic", "medic"]
  unitIds?: number[]; // ex: [1]
  limit?: number;
  className?: string;
};

export default function AionCard({
  dataUrl = "/data/aion_meta.json",
  ...
  const [meta, setMeta] = useState<AionMeta | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const m = mRes.ok ? ((await mRes.json()) as AionMeta) : {};
    ...
    >
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <aside className="relative rounded-xl border border-white/10 bg-white/5 overflow-hidden">
          ...
          {/* accent médical doux */}
          <div
            className="absolute inset-0 bg-emerald-300/5 mix-blend-overlay pointer-events-none"
            aria-hidden
          />
        </aside>
      </div>
